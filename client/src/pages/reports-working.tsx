...
import { CalendarDays, DollarSign, TrendingUp, Users, Wrench, AlertTriangle, Target } from "lucide-react";
...
export default function Reports() {
  ...
  const analytics = useMemo(() => {
    if (!Array.isArray(services) || !Array.isArray(customers) || !Array.isArray(serviceTypes)) {
      return null;
    }
    ...
    const totalRevenue = completedServicesWithRevenue.reduce((sum: number, service: any) => {
      const value = parseFloat(service.finalValue || service.estimatedValue || '0');
      return sum + value;
    }, 0);
    ...
    const serviceTypeStats = serviceTypes.map((serviceType: any) => {
      ...
      return {
        ...
        defaultPrice: parseFloat(serviceType.defaultPrice || '0')
      };
    ...
    const profitMargin = 35; // Estimated profit margin
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header 
          title={selectedCustomer !== "all" && Array.isArray(customers) && customers.length > 0 ? 
            `Relatórios - ${customers.find((c: any) => c.id?.toString() === selectedCustomer)?.name || 'Cliente'}` : 
            "Relatórios Executivos"
          }
          subtitle="Análise completa dos dados do seu negócio"
        />
              ...
              <TabsContent value="services" className="space-y-6">
                <Card>
                  <CardHeader>
                    ...
                    <p className="text-sm text-gray-600">Performance detalhada de cada tipo de serviço</p>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {analytics.serviceTypeStats.map((type: any, index: number) => (
                        <div key={type.name} className="border border-gray-200 rounded-lg p-4">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                              <span className="text-gray-500 block">Receita:</span>
                              <div className="font-medium text-green-600">R$ {type.revenue.toFixed(2)}</div>
                            </div>
                            <div>
                              <span className="text-gray-500 block">Ticket Médio:</span>
                              <div className="font-medium">R$ {type.avgTicket.toFixed(2)}</div>
                            </div>
                            <div>
                              <span className="text-gray-500 block">Participação:</span>
                              <div className="font-medium">{type.percentage.toFixed(1)}%</div>
                            </div>
                            <div>
                              <span className="text-gray-500 block">Preço Padrão:</span>
                              <div className="font-medium">R$ {type.defaultPrice.toFixed(2)}</div>
                            </div>
                          </div>
                        </div>
                      ))}
              ...
              <TabsContent value="customers" className="space-y-6">
                <Card>
                  <CardHeader>
                    ...
                    <p className="text-sm text-gray-600">Clientes que mais contribuem para o faturamento</p>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
